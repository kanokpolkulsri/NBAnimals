#include <avr/io.h>
#include <avr/interrupt.h>  /* for sei() */
#include <util/delay.h>     /* for _delay_ms() */
#include <avr/pgmspace.h>   /* required by usbdrv.h */

#include "peri.h"
#include "usbdrv.h"

#define RQ_SET_LED         0
#define RQ_SET_LED_VALUE   1
#define RQ_GET_SWITCH      2
#define RQ_GET_LIGHT       3
#define RQ_GET_ACCELERO_X    4
#define RQ_GET_ACCELERO_Y    5
#define RQ_GET_ACCELERO_Z    6

/* ------------------------------------------------------------------------- */
/* ----------------------------- USB interface ----------------------------- */
/* ------------------------------------------------------------------------- */
usbMsgLen_t usbFunctionSetup(uint8_t data[8])
{
    usbRequest_t *rq = (void *)data;

    /* declared as static so they stay valid when usbFunctionSetup returns */
    static uint8_t switch_state;  
    static uint16_t light;
    static uint16_t acceleroX; 
    static uint16_t acceleroY;
    static uint16_t acceleroZ;   

    if (rq->bRequest == RQ_SET_LED)
    {
        uint8_t led_val = rq->wValue.bytes[0];
        uint8_t led_no  = rq->wIndex.bytes[0];
        set_led(led_no, led_val);
        return 0;
    }

    else if (rq->bRequest == RQ_GET_SWITCH)
    {
        switch_state = IS_SWITCH_PRESSED();

        /* point usbMsgPtr to the data to be returned to host */
        usbMsgPtr = &switch_state;

        /* return the number of bytes of data to be returned to host */
        return 1;
    }

    else if (rq->bRequest == RQ_GET_LIGHT) {
        light = get_light(); // light = read_adc(PC2);
        usbMsgPtr = &light;
        return 3;
    }
    if (rq->bRequest == RQ_SET_LED_VALUE)
    {
        uint8_t led_val = rq->wValue.bytes[0];
        set_led_value(led_val);     
    }
    if(rq->bRequest == RQ_GET_ACCELERO_X){
        acceleroX = get_accelero_X();
        usbMsgPtr = &acceleroX;
        return sizeof(acceleroX);
    }
    if(rq->bRequest == RQ_GET_ACCELERO_Y){
        acceleroY = get_accelero_Y();
        usbMsgPtr = &acceleroY;
        return sizeof(acceleroY);
    }
    if(rq->bRequest == RQ_GET_ACCELERO_Z){
        acceleroZ = get_accelero_Z();
        usbMsgPtr = &acceleroZ;
        return sizeof(acceleroZ);
    }

    /* default for not implemented requests: return no data back to host */
    return 0;
}

/* ------------------------------------------------------------------------- */
int main(void)
{
    init_peripheral();

    usbInit();

    /* enforce re-enumeration, do this while interrupts are disabled! */
    usbDeviceDisconnect();
    _delay_ms(300);
    usbDeviceConnect();

    /* enable global interrupts */
    sei();

    /* main event loop */
    for(;;)
    {
        usbPoll();
    }

    return 0;
}

/* ------------------------------------------------------------------------- */
